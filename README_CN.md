...
- [ ] 图片提取，支持可选 OCR 识别嵌入截图
- [ ] Excel 公式计算支持（可选引擎）
- [ ] Word 嵌套表格支持
- [ ] Word 页眉页脚提取
- [ ] 可配置的输出格式选项（Markdown 风格、表格样式等）

### 第二阶段：增强功能（中期）

- [ ] 图表转 Markdown（柱状图、折线图、饼图转为文字描述或 Mermaid 图表）
- [ ] SmartArt 文本结构保留
- [ ] 批量转换，带进度条和汇总报告
- [ ] 监听模式：文件变更时自动转换
- [ ] 插件系统：支持自定义转换器和输出格式化器

### 第三阶段：高级能力（远期）

- [ ] 加密文件支持（用户提供密码）
- [ ] PDF 转 Markdown（统一 Office + PDF 转换器）
- [ ] 语义化文档结构检测（自动生成目录、章节编号）
- [ ] 多语言标题检测改进
- [ ] Web API / 微服务模式，支持云端部署
